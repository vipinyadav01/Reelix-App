...
export function useAddStory() {
  ...
  async function uploadAsset(asset: ImagePicker.ImagePickerAsset, privacy: 'public' | 'close_friends' = 'public') {
    setIsUploading(true);
    try {
      ...
      await createStory({ storageId: storageId as any, mediaType, caption: '', privacy });
    } finally {
  ...
  async function openEditor(asset: ImagePicker.ImagePickerAsset) {
    ...
    router.push({ pathname: '/story/edit', params: { uri: asset.uri, type } } as any);
  }

  const addStory = useCallback(async (opts?: { source?: 'gallery' | 'camera' }) => {
    if (opts?.source) {
