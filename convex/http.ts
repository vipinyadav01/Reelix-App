...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    if (!webhookSecret) {
      ...
      throw new Error(
        "Missing CLERK_WEBHOOK_SECRET not found in environment variables",
      );
    }
    // check headers
    ...
    if (eventType === "user.created" || eventType === "user.updated") {
      ...
      console.log(
        `${eventType === "user.created" ? "Creating" : "Updating"} user with data:`,
        {
          email,
      ...
      try {
        ...
        console.error(
          `Error ${eventType === "user.created" ? "creating" : "updating"} user:`,
          error,
        );
        return new Response(
          `Error ${eventType === "user.created" ? "creating" : "updating"} user`,
